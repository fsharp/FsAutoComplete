...
type SourceTextExtensions =
  ...
  [<Extension>]
  static member inline AsStream(t: ISourceText) =
    let lines: byte[] = Array.init (t.GetLineCount()) (t.GetLineString >> System.Text.Encoding.UTF8.GetBytes) |> Array.concat
    let s = new MemoryStream(lines)
    s :> Stream
...
type FileSystem (actualFs: IFileSystem, tryFindFile: string<LocalPath> -> VolatileFile option, tryRemoveFile: string<LocalPath> -> unit) =
    let getContent (filename: string<LocalPath>) =
         filename
         |> tryFindFile
         |> Option.map (fun file -> file.Lines)
    ...
    static let hasIllegalPathChars (path: string) =
      for c in path do
        if Array.contains c unixInvalidPathChars || Array.contains c windowsInvalidPathChars then failwith $"Character {c} in path {path} is invalid"
    ...
    interface IFileSystem with
        ...
        member _.AssemblyLoad (assemblyName:Reflection.AssemblyName) =
          actualFs.AssemblyLoad assemblyName

        member _.AssemblyLoadFrom (fileName:string) =
          actualFs.AssemblyLoadFrom fileName

        member _.FileDelete (fileName:string) = actualFs.FileDelete(fileName)
        member _.FileStreamCreateShim (fileName:string) = actualFs.FileStreamCreateShim (fileName)
        member _.FileStreamWriteExistingShim (fileName:string) = actualFs.FileStreamWriteExistingShim(fileName)
        member _.ReadAllBytesShim (fileName:string)  = actualFs.ReadAllBytesShim fileName
        member _.SafeExists (fileName:string) = actualFs.SafeExists fileName
